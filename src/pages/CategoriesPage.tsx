...
Find everything you need to transform your space.</p>
      ...
      <main className="container mx-auto px-4 py-16">
        {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {Array.from({
          length: 12
        }).map((_, i) => <div key={i} className="group">
            ...
            {categories.map(category => <Link key={category.id} to={`/categories/${category.slug}`} className="group">
        ...
        {/* Call to Action */}
        {!isLoading && categories.length > 0 && <div className="text-center mt-16 py-12 bg-gradient-to-r from-secondary/20 to-secondary/30 rounded-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/products" className="inline-flex items-center px-6 py-3 bg-primary text-primary-foreground rounded-full font-medium hover:bg-primary/90 transition-colors">
                View All Products
                <ArrowRight className="w-4 h-4 ml-2" />
              </Link>
              <Link to="/contact" className="inline-flex items-center px-6 py-3 border border-primary text-primary rounded-full font-medium hover:bg-primary hover:text-primary-foreground transition-colors">
                Contact Us
              </Link>
            </div>
          </div>}
