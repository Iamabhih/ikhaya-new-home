-- Create sample reviews with placeholder user approach
-- First, let's modify the reviews table to make user_id nullable for sample data
...
BEGIN
  ...
  FOR i IN 1..array_length(sample_products, 1) LOOP
    ...
    VALUES (
      sample_products[i],
      NULL, -- Anonymous user for sample data
      5,
      'Excellent product!',
    ...
    IF i <= 5 THEN -- Only add for first 5 products
      ...
      VALUES (
        ...
        false -- Not verified for variety
      ) ON CONFLICT DO NOTHING;
    END IF;
